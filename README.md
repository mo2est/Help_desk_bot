...
- Чистый диалог: инлайн-кнопки завершённых шагов автоматически деактивируются — в чате не остаётся «висящих» кнопок Отмена/Назад
...
- Пошаговые диалоги (FSM) устойчивы к нетекстовому вводу: стикер или фото вместо текста не ломают сценарий — бот просит повторить
- Клавиатура предыдущего шага диалога снимается при переходе дальше, при отмене и при повторном `/start` (ошибки редактирования устаревших сообщений игнорируются)
...
```
2024-01-15 10:00:00,123 [INFO] __main__: Starting Help Desk Bot...
2024-01-15 10:00:00,124 [INFO] __main__: Admin IDs: [123456789]
```
...
├── README_TEMPLATE.md         # Переиспользуемый шаблон README для Telegram-ботов
│
├── app/
...
│   ├── utils.py               # Учёт FSM-промптов: снятие устаревших инлайн-клавиатур
│   │
│   ├── handlers/
...
questions
├── id          INTEGER PK
├── category_id INTEGER FK → categories.id (каскадное удаление вместе с категорией)
├── text        TEXT NOT NULL
